...
the human table prints it in full so it can be passed directly to `slopctl
revive`. It also shows the tmux server boot UUID and tmux session ID, so an old
`%N` remains identifiable after tmux reuses pane IDs. Grave IDs are
timestamp-ordered UUID v7 values generated by the stock `uuid` crate. The human
table shows relative `CREATED`, `DESTROYED`, and `REVIVED` times like `slopctl
ps`; `--json` retains exact Unix timestamps and the same complete identifiers.
...
the stable grave ID. Copying the `GRAVE_ID` value from `slopctl graveyard` is
always unambiguous. A grave can be revived once; killing the revived pane creates
a new record.
...
| `send_prompt`, `wait_for_reply`, `interrupt_pane` | Prompt a pane, wait for its completed reply, or interrupt it |
| `read_transcript` | Read user prompts and completed agent text |

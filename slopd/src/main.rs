...
async fn tmux_unset_pane_option(
    config: &libslop::SlopdConfig,
    pane_id: &str,
    option: &str,
) -> std::io::Result<std::process::ExitStatus> {
...
fn reduce_pane_state(
    ...
    match event {
        ...
        PaneStateEvent::Hook {
            event,
            notification_type,
        } => reduce_hook_event(event, *notification_type),
        ...
        PaneStateEvent::TranscriptRecord {
            ...
            match *record_type {
                ...
                "progress" => {
                    let hook_event = record.get("data").and_then(|d| {
                ...
                "system"
                    if record.get("subtype").and_then(|s| s.as_str()) == Some("turn_duration") =>
                {
...
async fn run_opencode_driver(
    ...
    // when the pane is killed.
    {
        ...
        tokio::spawn(async move {
            run_opencode_sse(
                client, session_id, pane_id, config, panes, event_tx, cancel, subagents,
            )
            .await
        });
    }
...
async fn reconcile_opencode_status(
    ...
    let target = match client.session_status(session_id).await {
        ...
        Ok(None) => match client.session_ids().await {
            ...
            Ok(ids) if ids.iter().any(|id| id == session_id) => {
                Some(libslop::PaneDetailedState::Ready)
            }
            _ => None,
        },
...
async fn reconcile_opencode_subagents(
    ...
    let remaining = {
        ...
        set.retain(|sid, added| {
            let working = map
                .get(sid)
                .map(opencode::status_is_working)
                .unwrap_or(false);
    ...
    for sid in stopped {
        debug!(
            "opencode pane {}: pruning stale subagent {} (not working in /session/status)",
            pane_id, sid
        );
        let _ = event_tx.send(libslop::Record {
    ...
    loop {
        ...
        match client.events().send().await {
            Ok(resp) => {
                ...
                if let Err(e) = read_opencode_sse(
                    resp,
                    &session_id,
                    &pane_id,
                    &config,
...
async fn read_opencode_sse(
    ...
    loop {
        ...
        while let Some(idx) = buf.find("\n\n") {
            ...
            let Some(payload) = extract_sse_data(&block) else {
                continue;
            };
            let Some(event) = opencode::event_from_line(&payload) else {
                continue;
            };
            ...
            if let Some(selected) = opencode::tui_selected_session(&event) {
                if selected != current_session {
                    debug!(
                        "opencode pane {}: TUI selected session {} (was {})",
                        pane_id, selected, current_session
                    );
            ...
            let is_child = ev_sid
                .as_deref()
                .is_some_and(|s| subagents.lock().unwrap().contains_key(s));
            if !is_main && !is_child {
                continue;
            }
...
async fn schedule_opencode_auto_continue(
    ...
    tokio::spawn(async move {
        ...
        if !delay.is_zero() {
            debug!(
                "opencode session.error: pane {} will retry in {:?}",
                pid, delay
            );
        ...
        debug!(
            "opencode session.error: retrying pane {} (attempt {})",
            pid, attempt
        );
        if client.send_message(&session_id, &prompt).await.is_ok()
            && let Some(s) = panes.get(&pid)
        {
            ...
            Ok(n) => {
                ...
                match serde_json::from_str::<serde_json::Value>(trimmed) {
                    Ok(record) => {
                        ...
                        {
                            debug!(
                                "transcript enqueue: notifying pending senders for pane {}",
                                pane_id
                            );
                        ...
                        let is_slash_command_record = backend == libslop::Backend::Claude
                            && match record_type.as_str() {
                                ...
                                "system" => {
                                    record.get("subtype").and_then(|v| v.as_str())
                                        == Some("local_command")
                                        && record
                                            .get("content")
                                            .and_then(|c| c.as_str())
    ...
    loop {
        ...
        match reader.read_line(&mut line).await? {
            ...
            bytes_read => {
                ...
                if let Ok(record) = serde_json::from_str::<serde_json::Value>(trimmed)
                    && let Some((event_type, payload)) = decode_transcript_record(backend, &record)
                {
                    if window.len() == limit {
                        window.pop_front();
                    }
...
impl ManagedPanes {
    fn new() -> Self {
        Self {
            inner: Arc::new(dashmap::DashSet::new()),
        }
    }
...
async fn load_managed_panes(
    config: &Arc<libslop::SlopdConfig>,
    managed: &ManagedPanes,
    event_tx: &EventTx,
    panes: &PaneMap,
) -> std::collections::HashSet<(std::path::PathBuf, libslop::Backend)> {
    ...
    for pane_id in String::from_utf8_lossy(&out.stdout)
        .lines()
        .map(str::trim)
        .filter(|s| !s.is_empty())
    {
        let opts = match tmux(config)
        ...
        panes
            .get_or_insert(pane_id)
            .mark_unbound_backend(recovered_backend);
        if recovered_backend.uses_injected_hooks() {
            hook_paths.insert((config.resolved_hook_path(&resolved), recovered_backend));
        }
        ...
        if let Err(error) = backend_lifecycle(recovered_backend)
        ...
        {
            warn!(
                "failed to recover {} pane {}: {}",
                recovered_backend.canonical_executable(),
                pane_id,
                error
            );
        }
...
fn tmux_hook_command(slopctl: &str, hook_name: &str, include_pane_id: bool) -> String {
    ...
    if include_pane_id {
        format!(
            "run-shell \"XDG_RUNTIME_DIR={} {} tmux-hook {} #{{hook_pane}} || true\"",
            runtime_str, slopctl, hook_name
        )
    } else {
        format!(
            "run-shell \"XDG_RUNTIME_DIR={} {} tmux-hook {} || true\"",
            runtime_str, slopctl, hook_name
        )
    }
}
...
async fn register_tmux_hooks(config: &libslop::SlopdConfig) {
    ...
    for &(hook_name, include_pane_id) in TMUX_HOOKS {
        ...
        for line in existing.lines() {
            ...
            let is_slopctl_hook = line.contains(&stale_marker) && !line.contains(&our_command);
            if !is_slopctl_hook {
                continue;
            }
            // Extract the array index from "hook-name[N] ...".
            if let Some(idx_str) = line
                .strip_prefix(&prefix_bracket)
...
fn record_pane_death(
    ...
    if let Some(out) = output_tail
        .as_ref()
        .filter(|_| cause.is_abnormal(exit_status))
    {
...
fn recent_hook(hook_log: &HookLog) -> Option<String> {
    let fresh =
        |at: Option<std::time::Instant>| at.is_some_and(|t| t.elapsed() <= HOOK_CORRELATION_WINDOW);
...
async fn reconcile_panes(
    ...
    let (present_ids, dead_panes): (
        std::collections::HashSet<String>,
        std::collections::HashMap<String, Option<i64>>,
    ) = match output {
        Ok(out) if out.status.success() => parse_list_panes(&String::from_utf8_lossy(&out.stdout)),
        Ok(out)
            if {
                let stderr = String::from_utf8_lossy(&out.stderr);
                stderr.contains("no server running on") || stderr.contains("can't find session:")
            } =>
        {
    ...
    for pane_id in candidates {
        ...
        // `after-kill-pane` and `window-unlinked` can trigger concurrent
        // reconciles for the same disappearance. Atomically claim teardown via
        // the managed set so only one task removes the identity snapshot and
        // emits PaneDestroyed; a second task would otherwise emit a duplicate
        // record with an empty identity.
        if !managed_panes.remove(&pane_id) {
            continue;
        }
        ...
        let cause = if server_gone {
            DeathCause::ServerGone
        } else {
            DeathCause::Vanished
        };
        let preceding_hook = (cause == DeathCause::Vanished)
            .then(|| recent_hook(hook_log))
            .flatten();
        record_pane_death(
            event_tx,
            lifecycle,
...
async fn handle_dead_pane(
    ...
    // Multiple lifecycle hooks can reconcile the same dead pane concurrently.
    // Claim it once before doing any async capture or cleanup.
    if !managed_panes.remove(pane_id) {
        return;
    }
...
async fn main() {
    ...
    let (config_tx, config_rx) =
        tokio::sync::watch::channel::<Arc<libslop::SlopdConfig>>(initial_config.clone());
    ...
    let config_generation: Arc<std::sync::atomic::AtomicU64> =
    ...
    if lock_result != 0 {
        ...
        if err.raw_os_error() == Some(libc::EWOULDBLOCK) {
            error!(
                "slopd is already running (lock file held: {})",
                lock_path.display()
            );
            std::process::exit(1);
        }
    ...
    for (hook_path, backend) in &recovered_hook_paths {
        if let Err(e) =
            libslop::inject_backend_hooks_into_file(hook_path, &config.hook_slopctl(), *backend)
        {
            warn!(
                "failed to re-inject hooks into {}: {}",
                hook_path.display(),
                e
            );
        }
    ...
    for (hook_path, backend) in shutdown_config.all_hook_paths() {
        if let Err(e) = libslop::remove_backend_hooks_from_file(&hook_path, backend) {
            warn!(
                "failed to remove hooks from {} on shutdown: {}",
                hook_path.display(),
                e
            );
        } else {
    ...
    while let Ok(Some(line)) = lines.next_line().await {
        ...
        match req.body {
            libslop::RequestBody::Subscribe { filters } => {
                ...
                if write_response(&writer, req.id, libslop::ResponseBody::Subscribed)
                    .await
                    .is_err()
                {
                    return;
                ...
                tokio::spawn(stream_events_owned(
                    rx,
                    Arc::clone(&writer),
                    req.id,
                    filters,
            ...
            libslop::RequestBody::SubscribeTranscript { pane_id, last_n } => {
                ...
                for (cursor, event_type, payload) in &records {
                    ...
                    if write_response(&writer, req.id, libslop::ResponseBody::Record(record))
                        .await
                        .is_err()
                    {
                        return;
                ...
                if write_response(&writer, req.id, libslop::ResponseBody::Record(replay_end))
                    .await
                    .is_err()
                {
                    return;
            ...
            libslop::RequestBody::Unsubscribe { subscription_id } => {
                if let Some(cancel) = subscriptions.remove(&subscription_id) {
                    ...
                    let _ = write_response(
                        &writer,
                        req.id,
                        libslop::ResponseBody::Unsubscribed { subscription_id },
                    )
                    .await;
                } else {
                    let _ = write_response(
...
fn parse_pane_options(stdout: &str) -> ParsedPaneOptions {
    ...
    for opt_line in stdout.lines() {
            ...
            || key == "@slopd_claude_session_id"
        {
            ...
            opencode_token = if val.is_empty() {
                None
            } else {
                Some(val.to_string())
            };
...
async fn reparent_children_of(
    ...
    for child_id in managed_panes.snapshot() {
        ...
        let new_ancestors: Vec<String> = ancestors
            .into_iter()
...
async fn list_panes(
    config: &libslop::SlopdConfig,
    managed_panes: &ManagedPanes,
) -> Result<Vec<libslop::PaneInfo>, String> {
    ...
    for pane_id in managed_panes.snapshot() {
        ...
        let opts = match opts_out {
            Ok(out) if out.status.success() => {
                parse_pane_options(&String::from_utf8_lossy(&out.stdout))
            }
            _ => continue,
        };
    ...
    let live_ids: std::collections::HashSet<String> =
        raw_panes.iter().map(|p| p.pane_id.clone()).collect();
    ...
    for mut raw in raw_panes {
        ...
        let needs_reparent = parent_pane_id
            .as_ref()
            .is_some_and(|p| !live_ids.contains(p.as_str()));

        if needs_reparent {
            // Walk ancestors to find the first one that is still alive.
            let new_ancestors: Vec<String> = raw
                .opts
                .ancestor_panes
                .iter()
...
async fn read_pane_manifest(manifest_path: &std::path::Path) -> Vec<libslop::PaneInfo> {
    let bytes = match tokio::fs::read(manifest_path).await {
        ...
        Err(e) => {
            warn!(
                "backup: failed to read manifest {}: {}",
                manifest_path.display(),
                e
            );
            return Vec::new();
    ...
    match serde_json::from_slice::<Vec<libslop::PaneInfo>>(&bytes) {
        ...
        Err(e) => {
            warn!(
                "backup: manifest {} is malformed ({}); ignoring",
                manifest_path.display(),
                e
            );
            Vec::new()
...
async fn spawn_pane(
    ...
    // Create a live shell first. `remain-on-exit` must be enabled before the
    // backend process starts: a backend that exits immediately can otherwise
    // disappear between `new-window` returning and `set-option`, losing both
    // its exit status and final screen.
    ...
    match output {
        Ok(out) if out.status.success() => {
            ...
            // Scope this to THIS pane only (`-p`) so it never leaks onto the
            // user's other windows on the shared default tmux server.
            match tmux_set_pane_option(config, &pane_id, "remain-on-exit", "on").await {
                ...
                Ok(status) => {
                    let _ = tmux(config)
                        ...
                        .status()
                        .await;
                    return Err(format!("tmux set-option remain-on-exit exited {}", status));
                }
                Err(e) => {
                    let _ = tmux(config)
                        .args(["kill-pane", "-t", &pane_id])
                        .status()
                        .await;
                    return Err(format!("tmux set-option remain-on-exit failed: {}", e));
                }
            }

            // Replace the placeholder only after crash retention is active.
            // Re-pass the cwd and environment because respawn-pane starts a new
            // process rather than inheriting the placeholder shell's process
            // environment.
            let mut cmd = tmux(config);
            cmd.args(["respawn-pane", "-k", "-t", &pane_id]);
            if let Some(ref dir) = spec.working_dir {
                cmd.args(["-c", dir]);
            }
            cmd.args([
                "-e",
                &format!("XDG_RUNTIME_DIR={}", xdg_runtime_dir.display()),
            ])
            .args(["-e", &format!("SLOPCTL={}", config.run.slopctl)]);
            if let Some(ref dir) = spec.config_dir {
                cmd.args([
                    "-e",
                    &format!("{}={}", spec.backend.config_dir_env_var(), dir.display()),
                ]);
            }
            ...
            // Spawn the resolved absolute path, not the bare program name, so
            // the launch never depends on the pane's own PATH.
            cmd.arg(&resolved)
                .args(spec.executable.args())
                .args(&spec.trailing_args);

            match cmd.output().await {
                Ok(out) if out.status.success() => Ok(pane_id),
                Ok(out) => {
                    let _ = tmux(config)
                        .args(["kill-pane", "-t", &pane_id])
                        .status()
                ...
                Err(e) => {
                    let _ = tmux(config)
                        ...
                        .status()
                        .await;
                    Err(format!("tmux respawn-pane failed: {}", e))
                }
            }
        }
        Ok(out) => Err(format!(
...
async fn restore_panes(
    ...
    for p in &skipped {
        info!(
            "backup: skipping pane {} (no recorded session id, nothing to resume)",
            p.pane_id
        );
    }
    ...
    let parent_of: std::collections::HashMap<String, String> = resumable
        .iter()
        .filter_map(|p| p.parent_pane_id.clone().map(|par| (p.pane_id.clone(), par)))
        .collect();
    let depth_of: std::collections::HashMap<String, usize> = resumable
        .iter()
        .map(|p| {
    ...
    let mut old_to_new: std::collections::HashMap<String, String> =
        std::collections::HashMap::new();
    let mut ancestors_of_new: std::collections::HashMap<String, Vec<String>> =
        std::collections::HashMap::new();
    ...
    for p in ordered {
        ...
        if !seen_sessions.insert(session_id.clone()) {
            info!(
                "backup: skipping pane {} — session {} already running or restored",
                old_id, session_id
            );
            continue;
        }
        ...
        let new_id = match backend_lifecycle(resolved.backend)
    ...
    match body {
        ...
        libslop::RequestBody::Hook {
            ...
            if let Some(raw_transcript_path) = hook_transcript_path(hook_backend, &event, &payload)
            {
                ...
                let already_tailing =
                    state.transcript_path.lock().unwrap().as_deref() == Some(transcript_path);
                if !already_tailing {
                    debug!(
                        "hook {}: starting transcript tail for pane {} path={}",
                        event, pane, transcript_path
                    );
                    if let Err(e) = tmux_set_pane_option(
                        config,
                        pane,
                    ...
                    {
                        warn!(
                            "failed to set @slopd_transcript_path on pane {}: {}",
                            pane, e
                        );
                    }
            ...
            if event == "SessionStart" {
                ...
                if let Some(session_id) = session_id {
                    ...
                    {
                        warn!(
                            "failed to set @slopd_claude_session_id on pane {}: {}",
                            pane, e
                        );
                    }
                }
            ...
            if event == "UserPromptSubmit" {
                debug!(
                    "UserPromptSubmit: notifying pending senders for pane {}",
                    pane
                );
                let pane_state = panes.get_or_insert(pane);
                pane_state.prompt_submitted.notify_waiters();
                ...
                {
                    debug!(
                        "UserPromptSubmit: pane {} is slopd's auto-continue, preserving retry counter",
                        pane
                    );
                } else {
        ...
        libslop::RequestBody::Fork {
            ...
            let (fork_args, new_session_id) = match backend_lifecycle(src.backend)
                .fork(ForkContext {
                    pane_id: &pane_id,
            ...
            let start_directory =
                start_directory.or_else(|| src.working_dir.as_ref().map(std::path::PathBuf::from));
        ...
        libslop::RequestBody::Run {
            ...
            let resolved = match config.resolve_account(requested_account.as_deref()) {
                Ok(mut resolved) => {
                    ...
                    if let Some(backend) = backend {
                        resolved.executable = match libslop::Backend::infer_from_program(
                            resolved.executable.program(),
                        ) {
                            Some(inferred) if inferred == backend => resolved.executable.clone(),
                            Some(_) => libslop::Executable::String(
                                backend.canonical_executable().to_string(),
                            ),
                            None => resolved.executable.clone(),
                        };
                        resolved.backend = backend;
            ...
            let spawn_cwd = effective_start_dir
                .clone()
                .unwrap_or_else(|| std::env::current_dir().unwrap_or_default());
            ...
            let output = spawn_pane(
                config,
                session_lock,
                &SpawnSpec {
                    working_dir: effective_start_dir
                        .as_ref()
            ...
            match output {
                Ok(pane_id) => {
                    ...
                    debug!(
                        "spawned {:?} ({}) in pane {}",
                        resolved.executable,
                        resolved.backend.canonical_executable(),
                        pane_id
                    );
                        ...
                        PreparedBackendRun::Opencode {
                            ...
                            let session_id = if let Some(sid) = resume_session.clone() {
                                    ...
                                    async {
                                        loop {
                                            match client.session_ids().await {
                                                Ok(ids) if ids.contains(&sid) => return,
                                                _ => {
                                                    tokio::time::sleep(
                                ...
                                .await
                                {
                                    ...
                                    Err(_) => {
                                        warn!(
                                            "opencode pane {}: timed out creating/discovering a session; state tracking will be limited",
                                            pane_id
                                        );
                                        String::new()
                                    }
                                ...
                                tokio::spawn(async move {
                                    for _ in 0..10 {
                                        ...
                                        if let Err(e) = client.select_session(&sid).await {
                                            debug!(
                                                "opencode pane {}: select-session attempt failed: {}",
                                                pane, e
                                            );
                                        }
                                        tokio::select! {
                    ...
                    if current_state == libslop::PaneDetailedState::BootingUp {
                        let new_state =
                            reduce_pane_state(&current_state, &PaneStateEvent::Init).unwrap();
                        set_pane_detailed_state(
                            config, &pane_id, &new_state, None, event_tx, panes,
                        )
                        .await;
                    }
                    if let Some(ref parent) = parent_pane_id {
                        ...
                        {
                            warn!(
                                "failed to set @slopd_ancestor_panes on pane {}: {}",
                                pane_id, e
                            );
                        }
                    }
        ...
        libslop::RequestBody::Send {
            ...
            if let SendTransport::Unavailable(backend) = send_transport {
                return libslop::ResponseBody::Error {
                    message: format!(
                        "{} pane {} runtime is still attaching",
                        backend.canonical_executable(),
                        pane_id
                    ),
                };
            }
                    ...
                    match current_state {
                        libslop::PaneDetailedState::BootingUp => {
                            ...
                            if remaining.is_zero() {
                                return libslop::ResponseBody::Error {
                                    message: format!(
                                        "timed out after {}s waiting for opencode pane {} to become ready",
                                        timeout_secs, pane_id
                                    ),
                                };
                            }
                            tokio::time::sleep(
                                remaining.min(std::time::Duration::from_millis(300)),
                            )
                            .await;
                        }
                        libslop::PaneDetailedState::AwaitingInputPermission
                        ...
                        match tokio::time::timeout(remaining, async {
                            loop {
                                match state_rx.recv().await {
                                    Ok(ev)
                                        if ev.event_type == "DetailedStateChange"
                                            && ev.pane_id.as_deref() == Some(&pane_id) =>
                                    {
                                        if ev.payload.get("detailed_state").and_then(|v| v.as_str())
                                            == Some("ready")
                                        {
                                            return;
                                        }
            ...
            match result {
                Err(e) => libslop::ResponseBody::Error {
                    message: e.to_string(),
                },
                ...
                Ok(_) => {
                    ...
                    loop {
                        ...
                        if remaining.is_zero() {
                            break libslop::ResponseBody::Error {
                                message: format!(
                                    "timed out after {}s waiting for UserPromptSubmit on pane {}",
                                    timeout_secs, pane_id
                                ),
                            };
                        }
                ...
                Ok(out) => {
                    ...
                    let tags: Vec<String> = stdout
                        ...
                        .filter_map(|line| {
                            ...
                            opt.strip_prefix(libslop::TAG_OPTION_PREFIX)
                                .map(|t| t.to_string())
                        })
                        .collect();
        ...
        libslop::RequestBody::ReadTranscript {
                    ...
                    Ok(mut records) => {
                        if let Some(before) = before_cursor {
                            records.retain(|record| {
                                record.cursor.is_some_and(|cursor| cursor < before)
                            });
                        }
            ...
            match transcript_path {
                None => libslop::ResponseBody::TranscriptPage { records: vec![] },
                Some(path) => {
                    let path = std::path::PathBuf::from(&path);
        ...
        | libslop::RequestBody::Unsubscribe { .. } => {
            ...
            unreachable!(
                "Subscribe/SubscribeTranscript/Unsubscribe should be handled before handle_request"
            )
        }
    }
}
...
mod tests {
    ...
    fn pane_id_sort_orders_numerically_then_unparseable_last() {
        ...
        ids.sort_by_key(|id| pane_id_sort_key(id));
    ...
    fn extract_resume_target_recognizes_every_spelling() {
        ...
        for args in [
            ...
            assert_eq!(
                extract_resume_target(&args).as_deref(),
                Some(id),
                "args: {:?}",
                args
            );
        ...
        assert_eq!(
            extract_resume_target(&["--port".to_string(), "8080".to_string()]),
            None
        );
    ...
    fn strip_resume_flags_removes_flag_and_value_preserving_the_rest() {
        ...
        assert_eq!(
            strip_resume_flags(vec!["-s".to_string(), id.to_string()]),
            Vec::<String>::new()
        );
        assert_eq!(
            strip_resume_flags(vec![format!("--session={id}")]),
            Vec::<String>::new()
        );
    ...
    fn retry_next_increments_attempt_until_cap_then_stops() {
        ...
        assert_eq!(
            s2.next_send_at,
            now + tokio::time::Duration::from_millis(200)
        );

        // Third failure → attempt 3 exceeds cap → give up.
        assert!(
            RetryState::next(Some(&s2), &p, now).is_none(),
    ...
    fn retry_matches_only_its_own_scheduled_attempt() {
        ...
        assert!(
            !s.matches(1, now),
            "different scheduled time must not match"
        );
    }
    ...
    fn parse_list_panes_separates_live_and_dead() {
        ...
        assert!(
            !dead.contains_key("%26"),
            "a live pane must not be marked dead"
        );
    }
    ...
    fn codex_subagent_start_does_not_replace_the_main_transcript() {
        ...
        assert_eq!(
            hook_transcript_path(libslop::Backend::Codex, "SubagentStart", &payload,),
            None,
        );
        assert_eq!(
